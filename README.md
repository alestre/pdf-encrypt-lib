...
```js
await encryptPdf(bytes, 'password', { encryptMetadata: false });
```
...
- `decryptPdf(bytes, password) -> Promise<{ bytes: Uint8Array, owner: boolean, permissions: number, permissionsValid: boolean, encryptMetadata: boolean }>`
