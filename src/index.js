 ...
 * @returns {Promise<{ bytes: Uint8Array, owner: boolean, permissions: number, permissionsValid: boolean, encryptMetadata: boolean }>}
...
export async function decryptPdf(bytes, password) {
    ...
    let U48, O48, UE32, OE32, P, Perms32, encryptMetadata;
    try {
    ...
    return { bytes: await doc.save({ useObjectStreams: false }), owner: auth.owner, permissions: P, permissionsValid, encryptMetadata };
}
 ...
 * Original document permissions and encryptMetadata choice are preserved
 * unless overridden via `options.permissions` / `options.encryptMetadata`.
 *
...
export async function changePdfPassword(bytes, oldPassword, newPassword, options = {}) {
    ...
    return encryptPdf(plain.bytes, newPassword, { permissions: plain.permissions, encryptMetadata: plain.encryptMetadata, ...options });
}
